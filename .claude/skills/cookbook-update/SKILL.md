...
Read `.claude/sdlc.json` at the project root. The `cookbook` field points to the implementation cookbook file. The `cookbookRepo` field (optional) points to the repo where the cookbook lives if different from the current repo.
