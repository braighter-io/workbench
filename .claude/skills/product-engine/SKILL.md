...
Load `.claude/product-engine-state.json` if it exists (resume mode).
...
1. Read `.claude/sdlc.json` — stop if missing
2. Read `.claude/product-engine-state.json` if present:
...
Path: `.claude/product-engine-state.json`
